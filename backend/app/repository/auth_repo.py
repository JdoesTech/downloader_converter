...
class JWTRepo:
    ...
    def generate_token(self, expires_delta: Optional[timedelta]=None):
        ...
        encode_jwt = jwt.encode(to_encode, SECRET_KEY, algorithms=ALGORITHM)
        
        return encode_jwt
    
    def decode_token(self):
        try:
            decode_token = jwt.decode(self.decode, SECRET_KEY, algorithms=[ALGORITHM])
...
class JWTBearer(HTTPBearer):
    ...
    def verify_jwt(self, jwt_token:str):
        return True if jwt.decode(self.token, SECRET_KEY, algorithms=[ALGORITHM]) is not None else False